...
| 통화 표시 | 원화(KRW) 중심 | 원화/현지통화 토글 전환 |
| 환전 관리 | 없음 | 환전 지갑 (Wallet) 기능 |
| 결제 방식 | 구분 없음 | 지갑/카드 구분 |
| 지출 계산 | 단순 합계 | 환전 + 카드 = 총 지출 |
...
[결산]
- 총 지출: ₩1,015,000 + ₩153,500 = ₩1,168,500
- 남은 지갑: €700 - €450 = €250 (≈₩362,500)
- 실제 소비: €450 + ₩153,500 ≈ ₩805,000
```
...
| 유형 | 표시 방식 |
|------|----------|
| 환전 | `💶 EUR 환전 +€200 · ₩290,000` |
| 지갑 결제 | `☕ 카페 €4.50 · 지갑 (잔액 €245.50)` |
| 카드 결제 | `🚇 메트로 ₩3,500 · 카드` |

- **같은 날 여러 나라 방문 가능** (방문지 날짜는 참고용)
- **환전도 해당 날짜의 내역에 표시**
- **지갑 결제 시 잔액 변화 표시**
...
- **원화/현지통화 토글**: 금액 표시 방식 전환 가능
- **총 지출 = 환전 총액 + 카드 결제액** (지갑 내 소비는 환전 시점에 이미 계산됨)
- 환전 지갑: 잔액 / 환전 총액, 사용률 표시
- 오늘 내역에 **환전 내역도 표시**
- 지갑 결제 시 **지갑 잔액 함께 표시**
- [+] 버튼: 지출 입력 모달
...
**동작**:
- 캘린더에 국기로 방문 국가 표시 (같은 날 여러 국가 가능)
...
**지출 계산 로직**:
- **총 지출** = 환전 총액 + 카드 결제액 (원화 기준)
- **남은 지갑** = 각 지갑별 잔액 합계
- **실제 소비** = 지갑에서 사용한 금액 + 카드 결제액 (물건/서비스 구매)
- 환전은 "돈을 쓴 것"으로 간주, 지갑 내 소비는 이미 환전 시점에 계산됨
